...
timePerDivSlider = Slider(ax = plt.axes([0.4, 0.10, 0.4, 0.05]), label = "Time/div slider (ms)", valmin = 1, valmax = 10000, valinit = timePerDiv) # interactive time/div Slider object, plt.axes() is a function from the plt (pyplot) module that returns an axes object

timePerDivSlider.on_changed(timePerDiv) # event driven (when adjusted using mouse), therefore no need to be put inside loop, also when this method is used, timePerDiv changes

def adjustTimePerDiv (timePerDiv): 
   
   totalTime = timePerDiv * 10 # oscilloscopes usually have 10 horizontal divs
...
while 1 == 1:
   ...
   if len(voltSamples) == voltSampleSize: # FIXME: when time/div is smaller, waveform does not update automatically because voltSampleSize is much higher than what can be displayed on screen
   ...
   axes.plot(timeSamples, voltSamples)  # FIXME: sometimes Y-scale is completely off due to erroneous readings (Voltage reading in tens or hundreds) # need to plot timeSamples too to get the graph changing with horizontal adjustment
   ...
   # axes.text(x = 0.5, y = -0.05, s = str(timePerDiv) + " ms / div", transform = axes.transAxes, ha = "center", bbox = dict (boxstyle = "square", facecolor = "white", edgecolor = "black")) # a dict constructor (not like regular list syntax) is assigned to bbox (bounding box), transform ensures label stays in same relative location even if data changes or zoomed in, hence relative coordinates are passed like x = 0.5 (middle of x-axis) and y = -0.05 (slightly below y-axis) # FIXME I dont know why this line is still needed to display the slider 
